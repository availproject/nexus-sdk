...
export class Wallet extends EventEmitter implements EthereumProvider {
  private wallet: ethers.Wallet;
  private rpcProvider: ethers.JsonRpcProvider;
  private cachedChainId?: bigint;
  private chains: [number, string][];

  constructor(wallet: ethers.Wallet, chains: [number, string][]) {
    super();
  ...
  private async handleSendTransaction(rawTx: Record<string, unknown>) {
    ...
    const txRequest: ethers.TransactionRequest = {
      ...
      data: (rawTx.data as string) ?? undefined,
      ...
      gasLimit: rawTx.gas
        ...
        : undefined,
      maxFeePerGas: rawTx.maxFeePerGas ? BigInt(rawTx.maxFeePerGas as string) : undefined,
  ...
  private async handlePersonalOrEthSign(method: string, params: unknown[]): Promise<string> {
    if (params.length < 1) {
