...
Minimal Vite + TypeScript project that swaps into a fixed amount of USDT on Arbitrum using the Nexus SDK `swapWithExactOut` method.

## Prerequisites

- Node 18+ and npm
- EIP-1193 wallet (MetaMask, Rabby, Rainbow, Brave, Phantom) funded on Arbitrum **mainnet** (default config)
...
- Uses the preset `swapParams` in `src/nexus.ts` to request `toAmount` of USDT on Arbitrum.
