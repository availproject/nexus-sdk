...
export async function mainScreen(balances: UserAssetDatum[], onButtonClick: () => {}) {
  ...
  // Swap Information
  {
    ...
    pre.textContent +=
      '\n\tAmount: ' +
      +swapParams.toAmount.toString() / 1_000_000 +
      ' ' +
      findTokenMetadata(swapParams.toTokenAddress)?.symbol;
...
export function swapScreen(step: SwapStepType) {
  ...
  label.textContent = 'Swap with Exact Out Completed Step: ' + step.type;
  app.appendChild(label);
}
