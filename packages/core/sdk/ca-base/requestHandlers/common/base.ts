...
abstract class BaseRequest implements IRequestHandler {
  ...
  async processRFF(intent: Intent) {
    const cosmosAddress = (await this.input.cosmosWallet.getAccounts())[0].address;

    const { msgBasicCosmos, omniversalRFF, signatureData, sources, universes } =
      await createRFFromIntent(
        intent,
        {
          chainList: this.input.chainList,
          cosmos: {
            address: cosmosAddress,
            client: this.input.cosmosWallet,
          },
          evm: this.input.evm,
