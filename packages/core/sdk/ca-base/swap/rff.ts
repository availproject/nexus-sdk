...
import { ChainListType, Intent, NetworkConfig } from '@nexus/commons';
...
import { BridgeAsset, EoaToEphemeralCallMap, RFFDepositCallMap, Tx } from '@nexus/commons';
import { cosmosCreateDoubleCheckTx, cosmosCreateRFF } from './utils';
...
export const createIntent = ({
  ...
  for (const asset of assets) {
    ...
    if (
      Decimal.add(asset.eoaBalance.toString(), asset.ephemeralBalance.toString()).lte(
        unaccountedBalance2,
      )
    ) {
    ...
    if (allowance < source.value) {
      const allowanceTx = {
        data: encodeFunctionData({
          abi: ERC20ABI,
          args: [config.chainList.getVaultContractAddress(Number(source.chainID)), maxUint256],
