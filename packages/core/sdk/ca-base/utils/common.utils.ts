...
const createRequestEVMSignature = async (
  ...
  client: PrivateKeyAccount | WalletClient,
) => {
...
export class UserAsset {
  ...
  iterate(feeStore: FeeStore, sourceChains?: number[]) {
    let breakdown = this.value.breakdown;
    if (sourceChains && sourceChains.length) {
      breakdown = breakdown.filter((b) => sourceChains.includes(b.chain.id));
    }
...
class UserAssets {
  ...
  getChainCountWithBalance(asset?: UserAssetDatum) {
    return asset?.breakdown.filter((b) => new Decimal(b.balance).greaterThan(0)).length ?? 0;
  }
