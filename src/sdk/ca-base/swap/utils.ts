...
export const convertToEVMAddress = (address: Hex | Uint8Array) => {
  let _bytesAddress: Uint8Array = address as Uint8Array;
  if (typeof address === 'string') {
    _bytesAddress = toBytes(address);
  }

  if (_bytesAddress.length === 20) {
    return toHex(_bytesAddress);
  }

  if (_bytesAddress.length === 32) {
    return toHex(_bytesAddress.subarray(12));
  }
