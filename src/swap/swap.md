...
| **Bebop** | `buyTokens.minimumAmount` | `receiver_address` | calls Aggregation (`jam/{chain}/v2/quote`) and RFQ (`pmm/{chain}/v3/quote`) in parallel; `taker_address=userAddress`; RFQ adds `skip_validation=true`; EXACT_OUT uses `buy_amounts`; addresses **checksummed** (`getAddress`) | compares valid top-level responses by protected output (EXACT_IN) or required input (EXACT_OUT), and keeps a quote when the other API fails; RFQ can be disabled with `new BebopAggregator(getQuote, { useRfq: false })`; missing `priceUsd` ⇒ `value=0`, backfilled from a sibling |
