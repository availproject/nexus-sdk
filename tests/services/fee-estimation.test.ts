...
vi.mock('../../src/services/gas-pricing', () => ({
...
import { estimateTotalFees, type TxWithGas } from '../../src/services/fee-estimation';
...
describe('estimateTotalFees', () => {
  ...
  it('adds Arbitrum L1 gas units for raw estimates before buffering the gas limit', async () => {
    ...
    const [fee] = await estimateTotalFees(client, 42161, [
      {
        tx: {
  ...
  it('returns a separate L1 fee for OP Stack estimates', async () => {
    ...
    const [fee] = await estimateTotalFees(client, 8453, [
      {
        tx: {
          to: '0x1111111111111111111111111111111111111111',
          data: '0x1234',
  ...
  it('has no separate L1 fee on default fee models', async () => {
    const [fee] = await estimateTotalFees(client, 1, [
      {
        tx: {
          to: '0x1111111111111111111111111111111111111111',
          data: '0x1234',
